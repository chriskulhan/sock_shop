...
#if the cart isn't in session yet: 
@app.before_request
def before_request():
...
# home function sets up the homepage, so it gets the products and the 
#categories, then adds them to the index.html file (serves as template for the homepage)
...
#product_detail() function sets up the products_details page
#It displays the product details for a specific product id
...
#Route for add to cart page (this route will be activated when the user clicks the "Add to Cart" button)
#add_to_cart() function sets up the add_to_cart page, with product_id, quantity, and custom_text as the parameters
#the text the user wants on their socks
